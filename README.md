<h1 align="center">Hey 👋, I'm Allam Bhanu Prakash</h1>
<h3 align="center">Data Science Enthusiast • ML Engineer in the Making • B.Tech CSE (Data Science)</h3>

<p align="center">
  <img src="./banner.png" alt="Banner"/>
</p>

---

## 👨‍💻 About Me

- 🎓 B.Tech in Computer Science Engineering (Data Science) at Malla Reddy Engineering College (2022–2026)  
- 🤖 Passionate about Machine Learning, AI, NLP, and Data Analytics  
- 📊 Experienced in building ML models, dashboards, sentiment analysis systems, and prediction pipelines  
- 🚀 Interested in AI applications in finance, healthcare, and real-world automation systems  
- 💡 Currently focusing on Data Science, Deep Learning, and production-ready ML projects  

---

## 🧠 Tech Stack

### 💻 Languages
- Python, SQL, C, Java, HTML/CSS  

### 🤖 Machine Learning & AI
- Scikit-learn, TensorFlow, NLP, CNN, RandomForest, Regression, Classification  

### 📊 Data Science & Visualization
- Pandas, NumPy, Matplotlib, Seaborn, Power BI, Tableau  

### 🗄 Databases
- MySQL, MongoDB  

### 🛠 Tools & Platforms
- Streamlit, Git, GitHub, Jupyter Notebook, VS Code, Excel  

---

# 🏗 Featured Projects

## 📈 Trader Behavior & Bitcoin Market Sentiment Analysis
- Built an end-to-end data science project analyzing trader performance against Bitcoin Fear & Greed Index sentiment.
- Performed data cleaning, feature engineering, EDA, and machine learning modeling using RandomForest.
- Developed an interactive Streamlit dashboard for visualizing trader insights and sentiment impact.
- Focused on behavioral analytics and financial market sentiment prediction.

🔗 GitHub: https://github.com/bhanuyadav66

---

## 🐄 Livestock Health Disease Prediction System
- Developed an AI-powered livestock disease prediction system using machine learning techniques.
- Implemented symptom-based prediction models to help farmers identify diseases early.
- Built a prediction pipeline with preprocessing, model training, and result visualization.
- Focused on real-world agricultural and healthcare AI applications.

🔗 GitHub: [link](https://github.com/bhanuyadav66/livestock-marketplace)

---

## 🚦 Traffic Sign Classifier
- CNN-based deep learning model achieving 99%+ accuracy on German Traffic Sign dataset.
- Built using TensorFlow and Keras with a Tkinter GUI for real-time predictions.
- Implemented image preprocessing and deep learning classification pipeline.

🔗 GitHub: https://github.com/bhanuyadav66/Traffic-Sign-Classifier

---

## 🚗 Road Accident Risk Predictor
- Machine learning project predicting accident likelihood using weather, location, and traffic-related features.
- Performed data cleaning, feature engineering, EDA, and model evaluation.
- Built visualizations to identify accident risk patterns and contributing factors.

🔗 GitHub: https://github.com/bhanuyadav66/Road-Accident-Prediction

---

## 💬 Amazon Reviews Sentiment Analyzer
- NLP-based sentiment analysis system classifying customer reviews as positive or negative.
- Implemented preprocessing, tokenization, stopword removal, and lemmatization.
- Applied machine learning algorithms for sentiment prediction and text analytics.

🔗 GitHub: https://github.com/bhanuyadav66/Sentiment-Analysis-On-Poduct-Reviews

---

# 🏅 Certifications & Achievements

- 🐍 Kaggle Python Coder Badge  
- ☁️ Microsoft Azure AI Fundamentals  
- 🎓 IBM SkillsBuild Certifications:
  - AI Fundamentals
  - Data Fundamentals
  - Data Analysis Using Python  
- 📊 TATA Data Visualisation Virtual Experience Program  
- 📚 Big Data 101 & Hadoop 101 Certifications  

---

# 📈 Current Focus

- Machine Learning Engineering  
- Financial Data Analytics  
- NLP & Sentiment Analysis  
- Deep Learning Projects  
- Streamlit Dashboard Development  
- AI-powered Real-world Applications  

---

# 🌍 Languages

- English (Fluent)  
- Telugu (Native)  
- Hindi (Intermediate)  

---

# 🤝 Connect With Me

- 📍 Hyderabad, India  
- ✉️ Email: allambhanuprakash3413@gmail.com  
- 💼 LinkedIn: https://www.linkedin.com/in/allam-bhanu-yadav/  
- 💻 GitHub: https://github.com/bhanuyadav66  
- 🌐 Portfolio: https://my-portfolio-iota-six-38.vercel.app/  

---

<p align="center">
  ⭐ Turning data into intelligent solutions through AI & Machine Learning
</p>
